...
Tested with SimpleCov 1.0.3, Ruby 3.4, and two RSpec Multicore workers. Serial and parallel RSpec results match, parent boot coverage and both workers retain their exclusive coverage, and one `bundle exec rspec` run produces the combined HTML report.
...
RSpec::Multicore.on_worker_shutdown do
  # Calling result stores this worker's .resultset.json. Workers exit with
  # exit!, so inherited at_exit callbacks do not run.
  SimpleCov.result
end
...
RSpec.configure do |config|
  config.after(:suite) do
    ...
    # Code loaded before the workers fork belongs to the parent result set.
    SimpleCov.command_name("rspec-multicore-parent")
    SimpleCov.coverage_dir(File.join(worker_results, "parent"))
    SimpleCov.result
    resultsets << File.join(worker_results, "parent", ".resultset.json")
...
When multicore runs, worker shutdown hooks save partial results and the parent `after(:suite)` hook stores parent-only boot coverage before collating all result sets. If RSpec selects serial execution, no worker result sets exist and SimpleCov’s normal finalizer generates the report instead. `SimpleCov.result` is used without changing the formatter inside workers so the recipe also works with older SimpleCov releases that do not accept `SimpleCov.formatter false`.
